/* ===== Block Crush Daily — Shop & IAP System =====
...
(function() {
  ...
  let activeTab = 'coins'; // coins | gems | premium | upgrades
  ...
  function renderCoinShop(container) {
    ...
    html += '</div></div><div class="shop-section"><h3>🎲 Piece Styles</h3><div class="shop-grid">';
    for (const s of catalog.pieceStyles) {
      const owned = state.ownedPieceStyles.includes(s.id);
      const active = state.activePieceStyle === s.id;
      html += `
        <div class="shop-item ${owned ? 'owned' : ''} ${active ? 'active' : ''}" data-type="pieceStyle" data-id="${s.id}" data-price="${s.price}">
  ...
  function handleCoinPurchase(type, id, price, itemEl) {
    const state = ProgressionSystem.getState();
    // Check owned
    if (type === 'theme' && state.ownedThemes.includes(id)) {
      handleEquip('theme', id);
      return;
    }
    if (type === 'pieceStyle' && state.ownedPieceStyles.includes(id)) {
      handleEquip('pieceStyle', id);
      return;
    }

    if (!ProgressionSystem.spendCoins(price)) {
    ...
    } else if (type === 'pieceStyle') {
      state.ownedPieceStyles.push(id);
      state.activePieceStyle = id;
      ProgressionSystem.save();
  ...
  function handleEquip(type, id) {
    const state = ProgressionSystem.getState();
    if (type === 'theme' || type === 'pieceStyle') {
      const key = type === 'theme' ? 'activeTheme' : 'activePieceStyle';
      // If not owned, buy first
      const arr = type === 'theme' ? state.ownedThemes : state.ownedPieceStyles;
      if (!arr.includes(id)) return;
      state[key] = id;
      ProgressionSystem.save();
      showTab('coins');
      if (window.GameUI) window.GameUI.updateTheme();
  ...
  function renderGemShop(container) {
    ...
    container.querySelectorAll('.gem-pack .btn-buy').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (!data) return;
        // Simulate purchase — in production this goes through Google Play Billing
  ...
  function renderUpgradeStation(container) {
    ...
    html += `<div class="bonus-summary">
      <span>⚔️ Score: <strong>${(bonuses.scoreMult).toFixed(2)}x</strong></span>
      <span>🛡️ Shield: <strong>${Math.round(bonuses.shieldChance * 100)}%</strong></span>
      <span>❤️ Lives: <strong>${bonuses.extraLife + 1}</strong></span>
      <span>💰 Line Bonus: <strong>+${bonuses.clearBonus}</strong></span>
      <span>🔥 Combo: <strong>+${bonuses.comboBonus}</strong></span>
      <span>⚡ Start Energy: <strong>+${bonuses.startEnergy}</strong></span>
    </div>`;

    // Coin/gem balance for quick reference
    ...
    for (const [cat, tier] of Object.entries(tiers)) {
      ...
      if (currentData) {
        ...
        for (const [k, v] of Object.entries(currentData.bonus)) {
          if (v > 0) html += `<span class="bonus-chip">${v > 1 ? (k === 'scoreMult' ? `${v}x` : v) : ''} ${formatBonusName(k)}</span>`;
        }
        html += `</div>`;
      }

      if (nextData) {
        html += `<div class="upgrade-next">Next: <strong>${nextData.name}</strong> — `;
        const nextBonuses = [];
        for (const [k, v] of Object.entries(nextData.bonus)) {
          const curr = currentData?.bonus[k] || 0;
          if (v > curr) nextBonuses.push(`<span class="bonus-up">${formatBonusName(k)} ${v - curr > 0 ? `+${k === 'scoreMult' ? (v - curr).toFixed(1) + 'x' : v - curr}` : ''}</span>`);
        }
        html += nextBonuses.join(' ') + `</div>`;
      }
      ...
      if (maxed) {
        ...
        const hasEnoughCoins = state.coins >= costs.coins;
  ...
  function renderPremiumShop(container) {
    ...
    // Legendary Skins
    html += '<h4>⚔️ Legendary Weapon Skins</h4><div class="shop-grid">';
    ...
    container.querySelectorAll('.iap-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        // Simulate IAP purchase — production would use Google Play Billing
  ...
  function formatBonusName(key) {
    const names = {
      scoreMult: 'Score', clearBonus: 'Clear Bonus',
      startEnergy: 'Energy', shieldChance: 'Shield',
      extraLife: 'Life', comboBonus: 'Combo'
    };
